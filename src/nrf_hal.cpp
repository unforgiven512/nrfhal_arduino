...
/*! \brief The SPI chip select pin used */
static int cs_pin = CS_PIN_DEFAULT;
...
static uint16_t hal_nrf_read_multibyte_reg(uint8_t reg, uint8_t *pbuf, uint8_t len);
static void hal_nrf_write_multibyte_reg(uint8_t reg, const uint8_t *pbuf, uint8_t length);
 ...
 *
 * \todo Abstract this code as to not be Arduino-specific.
 *
 * \bug This code is currently Arduino-specific.
 */
...
hal_nrf_output_power_t hal_nrf_get_output_power(void) {
	return ((hal_nrf_output_power_t)((hal_nrf_read_reg(RF_SETUP) >> RF_PWR0) & 0x03));
}
...
hal_nrf_datarate_t hal_nrf_get_datarate(void) {
	...
	return ((hal_nrf_datarate_t)(((rf_setup & (uint8_t)BIT(RF_DR_LOW)) << 1) | (rf_setup & (uint8_t)BIT(RF_DR_HIGH))));
}
...
hal_nrf_crc_mode_t hal_nrf_get_crc_mode(void) {
	...
	if ((config & (uint8_t)BIT(EN_CRC))) {
		return ((config & (uint8_t)BIT(CRCO)) ? HAL_NRF_CRC_16BIT : HAL_NRF_CRC_8BIT);
	} else {
...
void hal_nrf_set_auto_retr(uint8_t retr, uint16_t delay) {
	uint8_t setup_retr = (uint8_t)((((delay >> 8) & 0x0F) << 4) | (retr & 0x0F));
...
uint8_t hal_nrf_get_clear_irq_flags(void) {
	return (hal_nrf_write_reg(STATUS, ((uint8_t)(BIT(MAX_RT) | BIT(TX_DS) | BIT(RX_DR))) & (uint8_t)(BIT(MAX_RT) | BIT(TX_DS) | BIT(RX_DR))));
}
...
uint8_t hal_nrf_clear_irq_flags_get_status(void) {
	...
	uint8_t retval = hal_nrf_write_reg(STATUS, ((uint8_t)(BIT(MAX_RT) | BIT(TX_DS) | BIT(RX_DR))) & (uint8_t)(BIT(MAX_RT) | BIT(TX_DS) | BIT(RX_DR)));

	retval |= (hal_nrf_read_reg(STATUS) & (uint8_t)(~(BIT(MAX_RT) | BIT(TX_DS) | BIT(RX_DR))));
...
uint8_t hal_nrf_get_tx_fifo_status(void) {
	return ((hal_nrf_read_reg(FIFO_STATUS) & (uint8_t)(BIT(TX_FIFO_FULL) | BIT(TX_EMPTY))) >> 4);
}
...
uint8_t hal_nrf_get_rx_fifo_status(void) {
	return (hal_nrf_read_reg(FIFO_STATUS) & (uint8_t)(BIT(RX_FULL) | BIT(RX_EMPTY)));
}
...
uint8_t hal_nrf_get_transmit_attempts(void) {
	return (hal_nrf_read_reg(OBSERVE_TX) & (uint8_t)(BIT(3) | BIT(2) | BIT(1) | BIT(0)));
}

uint8_t hal_nrf_get_packet_lost_ctr(void) {
	return ((hal_nrf_read_reg(OBSERVE_TX) & (uint8_t)(BIT(7) | BIT(6) | BIT(5) | BIT(4))) >> 4);
}
...
bool hal_nrf_get_reuse_tx_status(void) {
	return (bool)((hal_nrf_read_reg(FIFO_STATUS) & (uint8_t)BIT(TX_REUSE)) >> TX_REUSE);
}
...
void hal_nrf_reuse_tx(void) {
	cs_enable();
	SPI.transfer(REUSE_TX_PL);		/*! \todo abstract SPI transfer */
	cs_disable();
}
...
void hal_nrf_flush_rx(void) {
	cs_enable();
	SPI.transfer(FLUSH_RX);			/*! \todo abstract SPI transfer */
	cs_disable();
}
...
void hal_nrf_flush_tx(void) {
	cs_enable();
	SPI.transfer(FLUSH_TX);			/*! \todo abstract SPI transfer */
	cs_disable();
}
...
uint8_t hal_nrf_nop(void) {
	...
	cs_enable();
	status = SPI.transfer(NOP);		/*! \todo abstract SPI transfer */
	cs_disable();
...
uint8_t hal_nrf_read_reg(uint8_t reg) {
	...
	/* --- perform SPI transaction --- */
	cs_enable();
	SPI.transfer(buf, 2);		/*! \todo abstract SPI transfer */
	cs_disable();
...
void hal_nrf_save_ctx(hal_nrf_ctx_t *p_ctx) {
	/* Zero out the array used to store the context */
	memset(p_ctx, 0, sizeof(*p_ctx));

	/* Read out the context from the radio registers, storing it into the array */
	...
	hal_nrf_read_multibyte_reg(HAL_NRF_PIPE0, p_ctx->rx_addr_p0, sizeof(p_ctx->rx_addr_p0));
	hal_nrf_read_multibyte_reg(HAL_NRF_PIPE1, p_ctx->rx_addr_p1, sizeof(p_ctx->rx_addr_p1));
	...
	hal_nrf_read_multibyte_reg(HAL_NRF_TX, p_ctx->tx_addr, sizeof(p_ctx->tx_addr));
...
/**
 * \brief Basic function write to a register
 *
 * Use this function to write a new (byte) value to a radio register.
 *
 * \param[in]	reg		The register to write
 * \param[in]	value	The (new) value to write to \p reg
 *
 * \return The value of the \b STATUS register.
 *
 * \todo Abstract this code as to not be Arduino-specific.
 *
 * \bug This code is currently Arduino-specific.
...
/**
 * \brief Basic function, read from a multibyte register
 *
 * Use this function to read multiple bytes from a multibyte radio register.
 *
 * \param[in]	reg		The multibyte register to read from
 * \param[out]	pbuf	A pointer to the buffer in which to store the read bytes to
 * \param[in]	len		The expected response length. \n
 *						Set to \b 0 to have the length selected automatically.
 *
 * \return pipe # of received data (MSB), if operation used by hal_nrf_read_rx_payload().
 * \return length of read data (LSB), either for hal_nrf_read_rx_payload() or for hal_nrf_get_address().
 *
 * \todo Abstract this code as to not be Arduino-specific.
 *
 * \bug This code is currently Arduino-specific.
...
static uint16_t hal_nrf_read_multibyte_reg(uint8_t reg, uint8_t *pbuf, uint8_t len) {
	...
	if (length > 0) {
		...
		/* --- perform SPI transaction --- */
		cs_enable();
		SPI.transfer(buf, length + 1);		/*! \todo abstract SPI transfer */
		cs_disable();

		memcpy(pbuf, &buf[1], length);
	}
...
/**
 * \brief Basic function, write to a multibyte register
 *
 * Use this function to write multiple bytes to a multibyte radio register.
 *
 * \param[in]	reg		The register to write
 * \param[in]	pbuf	A pointer to the buffer in which data to write is located
